...
var _lodashDebounce = require("/node_modules/lodash.debounce");
var _lodashDebounceDefault = parcelHelpers.interopDefault(_lodashDebounce);
var _notiflix = require("notiflix");
var _fetchCountries = require("/src/js/fetchCountries");
const DEBOUNCE_DELAY = 300;
const TIMEOUT_NOTIFICATION = 4000;
...
inPut.addEventListener("input", (0, _lodashDebounceDefault.default)(onSearchCountry, DEBOUNCE_DELAY));
/**function */ function onSearchCountry(event) {
    const valueInput = event.target.value.trim();
    if (valueInput.length === 1) {
        (0, _notiflix.Notify).warning("At least 2 letters must be entered to search", {
            timeout: TIMEOUT_NOTIFICATION
        });
        return;
    } else if (valueInput.length === 0) {
        (0, _notiflix.Notify).info("Please start entering some country for searching", {
            timeout: TIMEOUT_NOTIFICATION
        });
    ...
    (0, _fetchCountries.fetchCountries)(valueInput).then(onRenderCountriesList).catch((error)=>{
...
function onRenderCountriesList(countries) {
    ...
    const markupCountriesList = countries.map((country)=>`<li class="country"><img src="${country.flags.svg}"
      alt="Flag of ${country.name.official}" />
      <h1>${country.name.official}</h1></li>`).join("");
    ...
    if (numberCountriesFound === 1) {
        ...
        const markupInfoAboutCountry = countries.map((country)=>`<p><b>Capital: </b>${country.capital}</p>
         <p><b>Population: </b>${country.population}</p>
         <p><b>Languages: </b>${Object.values(country.languages)}</p>`).join("");
        infoAboutCountryEl.innerHTML = markupInfoAboutCountry;
        return;
    }
    if (numberCountriesFound > 10) (0, _notiflix.Notify).warning("Too many matches found. Please enter a more specific name", {
        timeout: TIMEOUT_NOTIFICATION
    });
    infoAboutCountryEl.innerHTML = "";
}

},{"/node_modules/lodash.debounce":"3JP5n","notiflix":"5WWYd","/src/js/fetchCountries":"dKuy9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"3JP5n":[function(require,module,exports) {
/**
...
function fetchCountries(findCountry) {
    return fetch(`https://restcountries.com/v3.1/name/${findCountry}?fields=name,capital,population,flags,languages`).then((response)=>{
...
},{}]},["cVgJb","ebWYT"], "ebWYT", "parcelRequired7c6")
